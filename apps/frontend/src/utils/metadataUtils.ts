...
export function hasStepData<K extends PropertyKey>(
    patient: Nullish<Patient>,
    stepKey: K
): patient is PatientDataWithStep<K> {
    if (!hasKey(patient, stepKey)) return false
    return typeof patient[stepKey] === 'object'
}

export const hasNotesForStep = (
    data: Patient,
    meta: Step
): data is PatientDataWithStep<(typeof meta)['key']> &
    Record<(typeof meta)['key'], Record<'notes', string>> => {
    if (!hasStepData(data, meta.key)) return false
    return hasKey(data[meta.key], 'notes')
}

export const getStepData = <T extends Patient>(
    data: Nullish<T>,
    key: string
): Nullish<Record<string, any>> => {
...
export const getFieldIndexByNumber = (fields: Field[], fieldNumber: number) =>
    fields.findIndex((field) => field.fieldNumber === fieldNumber)